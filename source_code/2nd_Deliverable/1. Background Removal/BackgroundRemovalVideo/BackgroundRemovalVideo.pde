...
void setup() {
  size(1028, 720);
  video = new Capture(this, 1028, 720, "pipeline:autovideosrc");
  video.start();

  // Create an empty image the same size as the video
  backgroundImage = createImage(video.width, video.height, RGB);
  mybackgroundReplace = new Movie(this, "Golden.mp4");
  mybackgroundReplace.loop();
}
...
void movieEvent(Movie mybackgroundReplace) {
  mybackgroundReplace.read();
}
...
void draw() {
  ...
  image(mybackgroundReplace, 0, 0, width, height);
  ...
  loadPixels();
  video.loadPixels(); 
  mybackgroundReplace.loadPixels();
